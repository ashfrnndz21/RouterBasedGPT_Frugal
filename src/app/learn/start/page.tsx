'use client'
// src/app/learn/start/page.tsx - Persona onboarding
    ...
    <div className="min-h-screen bg-[#0a0a0f] px-5 py-8">
      <div className="max-w-sm mx-auto">
        <div className="flex items-center gap-2 justify-center mb-8">
          <div className="w-2 h-2 rounded-full bg-[#4F8EF7] shadow-[0_0_10px_#4F8EF7]" />
            ...
            return (
              <button key={p.id} onClick={() => setSelected(p.id)}
                ...
                <div className="flex items-center gap-3">
                  ...
                  {isSelected && (
                    <div className="ml-auto w-5 h-5 rounded-full flex items-center justify-center text-white text-[11px] font-bold"
                      style={{ background: p.color }}>✓</div>
                  )}
                </div>
                {isSelected && (
                  <p className="text-[11px] text-white/60 mt-2 leading-relaxed">{p.description}</p>
                )}
              </button>
            )
          })}
