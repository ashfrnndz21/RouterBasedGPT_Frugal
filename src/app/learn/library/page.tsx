    ...
    <div className="px-5 pb-4">
      ...
      {modules.slice(1).map(m => (
        <a key={m.id} href={m.url} target="_blank" rel="noopener noreferrer"
          className="flex items-start gap-3 p-3 bg-[#1c1c26] border border-white/[0.06] rounded-xl mb-2 hover:border-[#4F8EF7]/30 transition-colors cursor-pointer">
            ...
            {m.progress && m.progress.pct > 0 && (
              <>
                <div className="mt-1.5 h-0.5 bg-white/[0.05] rounded overflow-hidden">
                  <div className="h-full bg-[#4F8EF7]" style={{ width: `${m.progress.pct}%` }} />
                </div>
