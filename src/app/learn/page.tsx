'use client'
// src/app/learn/page.tsx — Learner Home Dashboard
...
function ScoreRing({ score, domain, color, emoji }: { score: number; domain: string; color: string; emoji: string }) {
  const pct = score / 300
  const r = 24
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        ...
        <div className="text-white/30 text-xs font-mono">Loading dashboard...</div>
      </div>
    )
  }
        ...
        <div className="mb-5">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
            {scores.map(s => {
              ...
              return (
                <ScoreRing key={s.domain} score={s.score} domain={meta.label} color={meta.color} emoji={meta.emoji} />
              )
            })}
          </div>
        </div>
      ...
      {inProgress.length > 0 && (
        <div className="mb-5">
          ...
          {inProgress.slice(0, 3).map(p => (
            <div key={p.moduleId}
              className="bg-[#13131a] border border-white/[0.06] rounded-xl p-3.5 mb-2.5 cursor-pointer hover:border-[#4F8EF7]/20 transition-all group">
